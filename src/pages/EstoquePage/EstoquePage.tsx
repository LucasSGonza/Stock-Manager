...
import { initialClothes, formatBRL } from "@/util";
import type { DClothing } from "@/types";
import { COLORS } from "@/util";

export function EstoquePage() {
  const [items, setItems] = useState<DClothing[]>(initialClothes);
  ...
  const handleSubmit = (data: Omit<DClothing, "id"> & { id?: number }) => {
    if (data.id != null) {
      setItems((prev) =>
        prev.map((i) => (i.id === data.id ? { ...i, ...data, id: data.id! } : i))
      );
    } else {
            ...
            >
              <Box sx={{ display: "flex", justifyContent: "space-between", gap: 1.5 }}>
                <Box>
                  ...
                  <Typography sx={{ fontSize: "1rem", mt: 0.25 }}>{item.name}</Typography>
                  <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: "block" }}>
                    {item.category} · Tam {item.size}
                  </Typography>
                </Box>
                <Box sx={{ textAlign: "right" }}>
                  <Typography sx={{ fontSize: "0.875rem" }}>{formatBRL(item.price)}</Typography>
                  <Typography variant="caption" color="text.secondary">
                    Qtd: {item.quantity}
                  </Typography>
                </Box>
